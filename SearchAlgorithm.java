public class SearchAlgorithm {
  ...
  public Schedule updatedBaseLine(SchedulingProblem problem, long deadline) {

    // get an empty solution to start from
    Schedule solution = problem.getEmptySchedule();

    for (int i = 0; i < problem.courses.size(); i++) {
      Course c = problem.courses.get(i); //Current Course
      boolean scheduled = false;
      for (int j = 0; j < c.timeSlotValues.length; j++) {
        if (scheduled) break;
        if (c.timeSlotValues[j] > 0) {
          for (int k = 0; k < problem.rooms.size(); k++) {
            if (solution.schedule[k][j] < 0) { //K is room. J is timeslot. 
              Room room=problem.rooms.get(k);
              if(c.enrolledStudents<room.capacity){
                solution.schedule[k][j] = i;
                scheduled = true;
                break;
              }
